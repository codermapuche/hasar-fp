...
namespace
{
	char Chars[] = 
	{
		...
		0xff, 0xd6, 0xdc, 0xf8, 0xa3, 0xd8, 0xd7, 0x83,
		0xe1, 0xed, 0xf3, 0xfa, 0xf1, 0xd1, 0xaa, 0xba,
		0xbf, 0xae, 0xac, 0xbd, 0xbc, 0xa1, 0xab, 0xbb,
		0xa6, 0xa6, 0xa6, 0xa6, 0xa6, 0xc1, 0xc2, 0xc0,
		0xa9, 0xa6, 0xa6, 0x2b, 0x2b, 0xa2, 0xa5, 0x2b,
		0x2b, 0x2d, 0x2d, 0x2b, 0x2d, 0x2b, 0xe3, 0xc3,
		0x2b, 0x2b, 0x2d, 0x2d, 0xa6, 0x2d, 0x2b, 0xa4,
		0xf0, 0xd0, 0xca, 0xcb, 0xc8, 0x69, 0xcd, 0xce,
		0xcf, 0x2b, 0x2b, 0xa6, 0x5f, 0xa6, 0xcc, 0xaf,
	...
	char Oems[] = {
		...
		0x5e, 0x25, 0x53, 0x3c, 0x4f, 0x3f, 0x5a, 0x3f,
		0x3f, 0x27, 0x27, 0x22, 0x22, 0x07, 0x2d, 0x2d,
		0x7e, 0x54, 0x73, 0x3e, 0x6f, 0x3f, 0x7a, 0x59,
...
void
LinuxOemToChar(const char *s1, char *s2)
{
	unsigned len = strlen(s1);

	for (unsigned i = 0; i < len; i++)
		s2[i] = Chars[unsigned (s1[i])];
	s2[len] = 0;
}
...
void
LinuxCharToOem(const char *s1, char *s2)
{
	unsigned len = strlen(s1);

	for (unsigned i = 0; i < len; i++)
		s2[i] = Oems[unsigned (s1[i])];
	s2[len] = 0;
}

#define CharToOem LinuxCharToOem
#define OemToChar LinuxOemToChar

#endif

void ConvertToASCII(const char *s1, char *s2)
{
	CharToOem(s1, s2);
}

void ConvertFromASCII(const char *s1, char *s2)
{
	OemToChar(s1, s2);
}
